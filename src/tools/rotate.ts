 ...
 */
export async function getPDFRotations(inputPath: string): Promise<Array<{ page: number; rotation: number }>> {
  try {
    ...
    throw new Error(`Failed to get PDF rotations: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}